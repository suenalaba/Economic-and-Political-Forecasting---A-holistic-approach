# ========================================================================================================
# Purpose:      Linear Regression & CART analysis for Political Stability Analysis - Fragile States Index
...
#For comparison to Argentina
FSI_data <- fread("FSI_data.csv")

#===============================================================================

#relevel factor variables, because order was loss during write and read
...
developed_countries_cart1[, `Fragility_Category (Year+1)` := cut(`FSI (Year+1)`, breaks=cut.off, include.lowest=T)]
levels(developed_countries_cart1$`Fragility_Category (Year+1)`) = c("Very sustainable","Sustainable","More Stable","Stable",
                                                    "Less Stable","Warning","Elevated Warning","High Warning",
                                                    "Alert","High Alert","Very High Alert")

developing_countries_cart1[, `Fragility_Category (Year+1)` := cut(`FSI (Year+1)`, breaks=cut.off, include.lowest=T)]
levels(developing_countries_cart1$`Fragility_Category (Year+1)`) = c("Very sustainable","Sustainable","More Stable","Stable",
                                                                    "Less Stable","Warning","Elevated Warning","High Warning",
                                                                    "Alert","High Alert","Very High Alert")

leastdeveloped_cart1[, `Fragility_Category (Year+1)` := cut(`FSI (Year+1)`, breaks=cut.off, include.lowest=T)]
levels(leastdeveloped_cart1$`Fragility_Category (Year+1)`) = c("Very sustainable","Sustainable","More Stable","Stable",
                                                                    "Less Stable","Warning","Elevated Warning","High Warning",
                                                                    "Alert","High Alert","Very High Alert")

transitioning_cart1[, `Fragility_Category (Year+1)` := cut(`FSI (Year+1)`, breaks=cut.off, include.lowest=T)]
levels(transitioning_cart1$`Fragility_Category (Year+1)`) = c("Very sustainable","Sustainable","More Stable","Stable",
                                                                    "Less Stable","Warning","Elevated Warning","High Warning",
                                                                    "Alert","High Alert","Very High Alert")
...
#visualise seasonal trends of outcome variable
ggplot(FSI_cart4[c(21:30,322:331,308:316,160:169),c(1:3)]) +
  aes(x=Year, y=FSI, group=`Country Name`, color=`Country Name`) + 
  geom_smooth() + 
  labs(title = "Fragile States Index across different economies") +
  xlab("Year") + 
  ylab("Fragile States Index(FSI)") + 
  scale_y_continuous(labels = scales::comma)


#data visualisation of Fragility Categories and their distribution over economy types
ggplot(data = FSI_cart4, aes(`Fragility_Category (Year+4)`, fill = economy_type)) +
  geom_bar(position = "fill") +
  labs(x = "Fragility Category (Year+4)", 
       y = "Proportion", 
       title = "Proportion of each economy type in different Fragility Categories")
...
#exploring data with reasonably high correlations with outcome variable
ggplot(FSI_cart4) +
  aes(x=`GDP Per Capita`, y=`FSI (Year+4)`, col = `economy_type`) + 
  geom_smooth() + 
  labs(title = "Fragile States Index across different economies") +
  xlab("GDP Per Capita") + 
  ylab("Fragile States Index(FSI) (Year+4)") + 
  scale_y_continuous(labels = scales::comma) 

#data exploration for Human Development Index
ggplot(FSI_cart4) +
  aes(x=`Human Development Index`, y=`FSI (Year+4)`, col = `economy_type`) + 
  geom_jitter() + 
  labs(title = "Fragile States Index across different economies") +
...
#data exploration for Life Satisfaction
ggplot(FSI_cart4) +
  aes(x=`Life_Satisfaction`, y=`FSI (Year+4)`, col = `economy_type`) + 
  geom_point() + 
  labs(title = "Fragile States Index across different economies") +
...
#data exploration for No Violence Index
ggplot(FSI_cart4) +
  aes(x=`No violence Index`, y=`FSI (Year+4)`, col = `economy_type`) + 
  geom_smooth() + 
  labs(title = "Fragile States Index across different economies") +
  xlab("No violence Index") + 
  ylab("Fragile States Index(FSI) (Year+4)") + 
  scale_y_continuous(labels = scales::comma) 
...
RMSE.trainmodel 
RMSE.test 
MAPE(predict.test, testset$`FSI (Year+1)`)
...
#In general given any random set of data, actual FSI(Y+1) will differ from forecasted FSI(Y+1) by 1.887529.
#MAPE is approximately 3.5% which is a highly accurate forecast.
...
# 4 data points are heavily misclassified, 1 predicts a very sustainable country to only be "more stable"
#the other predicts a sustainable country only as stable.
#Thirdly, a stable country is predicted to be in the "warning" category
#Lastly, a country that is "warning" is predicted as stable.

# Overall Accuracy
mean(cart.predict == testset$`Fragility_Category (Year+1)`)

#Overall model accuracy is 68.1%. On average, 31.9% data will be misclassified.
...
#In general,given any random set of data, actual FSI(Y+1) will differ from forecasted FSI(Y+1) by 1.383143
#There is a 1.7% Mean absolute percentage error indicating a highly accurate forecasting
#This shows that the model to forecast FSI 1 year into the future is quite strong.
#This is supported by the visualisation plots, which show that the actual values lie closely
#to the forecasted FSI(Y+1) represented by the red line.
...
par(mfrow = c(1,1))
rpart.plot(cart1, nn=T, trace=-1, main = "Maximal Tree for Fragility_Category (Year+1) for developing countries")
...
rpart.plot(cart2, nn=T, tweak  = 2.5, fallen.leaves=FALSE, box.palette = "RdYlGn", main = "Optimal Tree for Fragility_Category (Year+1) for developing countries")

prp(cart2,box.palette = "-RdYlGn") #to get better visual of decision making
...
#In general, given any random set of data, actual FSI(Y+1) will differ from forecasted FSI(Y+1) by 1.90
#The MAPE is 1.5% which is a highly accurate forecasting
...
cart1 <- rpart(`Fragility_Category (Year+1)` ~ . -FSI ,data = trainset,
               method = 'class', control = rpart.control(minsplit = 7, cp = 0))
par(mfrow = c(1,1))
rpart.plot(cart1, nn=T, tweak=1.5, box.palette=0, main = "Maximal Tree for Fragility_Category (Year+1) for least developed countries")
...
prp(cart2, box.palette = "RdYlGn") #to get a better view of the decision tree
...
table <- table(Testset.Actual = testset$`Fragility_Category (Year+1)`, cart.predict, deparse.level = 2)
table
#2 data points are predicted to be in elevated warning category when they should be at high alert.
#2 data points are predicted to be in high warning when they should be in high alert.
#1 data point should be in elevated warning but predicted as alert.
#1 data point should be in very high alert but predicted only as alert.
#6 heavy misclassified categories
...
#In general given any random set of data, actual FSI(Y+1) will differ from forecasted FSI(Y+1) by 1.407306
#There is 1.5% MAPE, which points towards a highly accurate forecasting
...
table <- table(Testset.Actual = testset$`Fragility_Category (Year+1)`, cart.predict, deparse.level = 2)
table
#There is a data point that is supposed to be high warning but only predicted as less stable.
#2 data points are predicted as less stable when it should be at elevated warning.
#1 data point is predicted as elevated warning when it should be less stable.
#1 data point is predicted as elevated warning when it should be alert.
#There are 5 heavy misclassifications.
...
#In general, given any random set of data, actual FSI(Y+4) will differ from forecasted FSI(Y+4) by 3.2793
#MAPE is 6.6% which still falls under the highly accurate forecasting category.
...
#In general, given any random set of data, actual FSI(Y+4) will differ from forecasted FSI(Y+4) by 2.98
#MAPE is 3.6% which is a highly accurate forecast.
#This shows that the model to forecast FSI 4 year into the future is quite strong, for developing countries
#This is supported by the visualisation plots, which show that the actual values lie closely
#to the forecasted FSI(Y+4) represented by the red line.
...
#In general, given any random set of data, actual FSI(Y+4) will differ from forecasted FSI(Y+4) by 3.138622
#The MAPE is 3% which is a highly accurate forecast.
...
#In general, given any random set of data, actual FSI(Y+4) will differ from forecasted FSI(Y+4) by 3.44
#MAPE is 3.5% which falls under the category of highly accurate forecasting
#This shows that the model to forecast FSI 4 year into the future is quite strong for transitioning countries
...
cart1 <- rpart(`Fragility_Category (Year+4)` ~ . ,data = trainset,
               method = 'class', control = rpart.control(minsplit = 9, cp = 0))
par(mfrow = c(1,1))
rpart.plot(cart1, nn=T, tweak=1.0, fallen.leaves =FALSE, box.palette="RdYlGn", 
          main = "Maximal Tree for Fragility_Category (Year+4)")
...
table
#1 heavy misclassification, when alert is predicted only as elevated warning. 
...
#end of analysis for political stability using FSI =============================
