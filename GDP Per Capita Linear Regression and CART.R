...
#This shows that the model to forecast is actually quite strong, given the large of GDP Per Capita range(527.1 to 88246)
...
rpart.plot(cart2, nn=T, tweak = 2, main = "Optimal Tree for GDP Per Capita PPP (Year+1) for Developed Countries")
...
#Taking into consideration the range of GDP Per Capita PPP for developed countries: 5544 to 101298
#This means an average of 4.53% error in forecasting, which is reasonably strong!
...
developing_prediction <- data.frame(Predicted = predict.test,  # Create data for ggplot2
                                   Actual = testset$`GDP Per Capita PPP (Year+1)`)
# Draw plot using ggplot2 package
developing_plot <- ggplot(developed_prediction,                                     
                         aes(x = Predicted,
                             y = Actual)) +
  geom_point() +
  geom_abline(intercept = 0,
...
#Min split is determined by testing which gives the least SE
set.seed(2004)
cart1 <- rpart(`GDP Per Capita PPP (Year+1)` ~ . ,data = trainset,
...
least_developed_prediction <- data.frame(Predicted = predict.test,  # Create data for ggplot2
                                   Actual = testset$`GDP Per Capita PPP (Year+1)`)
# Draw plot using ggplot2 package
least_developed_plot <- ggplot(developed_prediction,                                     
                         aes(x = Predicted,
                             y = Actual)) +
  geom_point() +
  geom_abline(intercept = 0,
...
#Taking into consideration the range of GDP Per Capita PPP for least developed countries: 429.9 to 38407.9
#This means an average of 8.58% error in forecasting, which is reasonably strong!
...
transition_prediction <- data.frame(Predicted = predict.test,  # Create data for ggplot2
                                   Actual = testset$`GDP Per Capita PPP (Year+1)`)
# Draw plot using ggplot2 package
transition_plot <- ggplot(developed_prediction,                                     
                         aes(x = Predicted,
                             y = Actual)) +
  geom_point() +
  geom_abline(intercept = 0,
...
#This shows that the model to forecast is actually quite strong, given the large of GDP Per Capita(1420 to 26074)
#This is supported by the visualisation plots, which show that the actual values lie closely
#to the forecasted GDP Per Capita(Y+1) represented by the red line.
...
#Taking into consideration the range of GDP Per Capita PPP for transitioning economies: 928.3 to 26073.9
#This means an average of 9.40% error in forecasting, which is reasonably strong!
